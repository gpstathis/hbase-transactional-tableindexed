...
class IndexedRegion extends TransactionalRegion {
    ...
    private HTableInterface getIndexTable(final IndexSpecification index) throws IOException {
        return tablePool.getTable(index.getIndexedTableName(super.getRegionInfo().getTableDesc().getName()));
    }
     ...
     */
    private boolean possiblyAppliesToIndex(final IndexSpecification indexSpec, final Put put) {
        for (List<KeyValue> familyPuts : put.getFamilyMap().values()) {
            for (KeyValue kv : familyPuts) {
                if (indexSpec.containsColumn(KeyValue.makeColumn(kv.getFamily(), kv.getQualifier()))) {
                    return true;
                }
            }
        }
    ...
    @Override
    public void delete(final Delete delete, final Integer lockid, final boolean writeToWAL) throws IOException {
        ...
        if (!getIndexes().isEmpty()) {
            ...
            for (IndexSpecification indexSpec : getIndexes()) {
                ...
                try {
                    if (indexDelete != null
                            && (indexPut == null || !Bytes.equals(indexDelete.getRow(), indexPut.getRow()))) {
                        ...
                        for (byte[] indexCol : indexSpec.getAdditionalColumns()) {
                            ...
                            if (famDeletes != null) {
                                for (KeyValue kv : famDeletes) {
                                    if (Bytes.equals(indexCol, KeyValue.makeColumn(kv.getFamily(), kv.getQualifier()))) {
                                        LOG
                                                .debug("Need to delete this specific column: "
                                                        + Bytes.toString(KeyValue.makeColumn(kv.getFamily(), kv
                                                                .getQualifier())));
                                        Delete columnDelete = new Delete(indexDelete.getRow());
                                        columnDelete.deleteColumns(kv.getFamily(), kv.getQualifier());
    ...
    private SortedMap<byte[], byte[]> convertToValueMap(final Result result) {
        ...
        if (list != null) {
            for (KeyValue kv : result.list()) {
                currentColumnValues.put(KeyValue.makeColumn(kv.getFamily(), kv.getQualifier()), kv.getValue());
            }
        }
        return currentColumnValues;
    }
